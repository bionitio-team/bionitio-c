 ...
 */
void printVersion() { printf("bionitio version %s\n", VERSION); }
...
int processFiles(FILE *logFile, int minlen, char **files, int fileCount) {
  ...
  bionitio_log(logFile, "processing %i file(s) with minlen %i...", fileCount,
               minlen);
  puts(HEADER);
  if (fileCount == 0) {
    ...
    if (result.total_sequences == 0 &&
        !result.is_empty) { // no sequences at all is an error
    ...
    if (result.sequences == 0) {
      ...
      fprintf(stdout, "stdin\t%lu\t%lu\t%lu\t%.0f\t%lu\n", result.sequences,
              result.bases, result.min, result.average, result.max);
    }
    ...
    for (int current = 0; current < fileCount; current++) {
      ...
      if (result.sequences == 0) {
        ...
        fprintf(stdout, "%s\t%lu\t%lu\t%lu\t%.0f\t%lu\n", files[current],
                result.sequences, result.bases, result.min, result.average,
                result.max);
      }
...
int main(int argc, char **argv) {
  ...
  while (1) {
    static struct option long_options[] = {{"log", required_argument, 0},
